...
function perform_layout_csg_operations(container_csg, child) {
    ...
    var matrix = compute_transformation_matrix(child.csg, 'base', container_csg, 'top', dx, -dy)
...
function construct(node, parent) {
    ...
    children.models.forEach(function(child) {
        perform_layout_csg_operations(container_csg, child)
    })
    ...
    if (container_csg.imaginary) {
        node.csg = container_csg //null
    } else {
        node.csg = container_csg
    }

    var cb = container_csg.getBounds()
